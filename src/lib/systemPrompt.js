...
❌ 悪い例（1行に詰め込む）：
\`\`\`
もし <(([絶対値 v] の (((ボール v) の [x座標 v]) - (x座標))) * (15)) > (([絶対値 v] の (((ボール v) の [y座標 v]) - (y座標))) * (24))> なら
\`\`\`

✅ 良い例（中間変数に分けて段階計算）：
\`\`\`
[差x v] を (((ボール v) の [x座標 v]) - (x座標)) にする
[差x v] を ([絶対値 v] の (差x)) にする
[差y v] を (((ボール v) の [y座標 v]) - (y座標)) にする
[差y v] を ([絶対値 v] の (差y)) にする
もし <((差x) * (15)) > ((差y) * (24))> なら
\`\`\`
...
([絶対値 v] の (0))
\`\`\`
※数学関数（絶対値／切り下げ／切り上げ／平方根など）は必ず「([絶対値 v] の (値))」と関数名が先のドロップダウン。「((値) の絶対値)」と後ろに付けると赤ブロックになる。
